...
for i in range(len(s)):
    ...
    proc_spec= np.vstack((x, norm_flx)) # 2D array. 1st row: restframe wavelength, 2nd row: corrected flux
    hdu= fits.PrimaryHDU(proc_spec) # HDU with flux and wavelngth to be saved as a fits file
    new_spec_name= './proc_data/spec-'+str(s['PLATE'][i])+'-'+str(s['MJD'][i])+'-'+str(s['FIBERID'][i]).zfill(4)+'_proc.fits' # name of new processed spectrum
